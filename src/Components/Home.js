import React from 'react';
import Lottie from  "lottie-react";
import SpaceBoy from "../LottieFiles/SpaceBoy.json";
import Typed from "./Typed.js";
import Tilt from 'react-parallax-tilt';
import Avatar from "../LottieFiles/BoyLook.json";
import {CiCoffeeCup} from "react-icons/ci";
import { NavLink } from 'react-router-dom';

const Home = () => {
  
  return (
    <div >
      <div className='HomePage'>

        <div className='HomeText' data-aos="fade-right"
          data-aos-duration="1000">
          <h1>Hi There!</h1>
          <h1>I'M <b>Prahlad Sharma</b></h1>
          <Typed/>   
          <button className="contact-submit">
            <NavLink to="/Contact" >Hire Me</NavLink>
            </button>
        </div>

        <Lottie 
          className="illustration" data-aos="fade-left"
          data-aos-duration="900"
          animationData={SpaceBoy} 
          loop={true} 
        />
      
      </div>
      
      <div className='AboutPage'data-aos="fade-right"
          data-aos-duration="1000">
        <div className='AboutText' >
          <h1 className='AboutTextHeading'> <b>Introduction</b></h1>
          <p>
            I love the process of changing a raw idea into a website or a product 
            that impacts lives. 
            I want to do work that challenges me as a developer & work that I can 
            be proud of.<br /><br />
            I know <b>Javascript</b> and a bit of <b>PHP</b> and i am working on a few 
            projects in the <b>MERN</b> stack.<br />
            I plan to learn <b>Next.js</b>, <b>Three.js</b> and<b> Typescript</b> in the near future. <br /><br />
            Also, I love <b>coffee</b> <CiCoffeeCup style={{scale:"1.5", rotate:"15deg"}}/>   
          </p>
        </div>
        <Lottie 
          className="illustration" data-aos="fade-left"
          data-aos-duration="900"
          animationData={Avatar} 
          loop={true} 
        />
      </div>
    </div>
  )
}

export default Home